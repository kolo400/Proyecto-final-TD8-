...
class RandomVerticalFlip(T.RandomVerticalFlip):
    def forward(self, image: Tensor,
                target: Optional[Dict[str, Tensor]] = None) -> Tuple[Tensor, Optional[Dict[str, Tensor]]]:
        if torch.rand(1) < self.p:
            image = F.vflip(image)  # Aplica el flip vertical
            if target is not None:
                _, height = F.get_image_size(image)  # Obtiene la altura de la imagen
                # Ajusta las coordenadas de las bounding boxes verticalmente
                target["boxes"][:, [1, 3]] = height - target["boxes"][:, [3, 1]]
                if "masks" in target:
                    target["masks"] = target["masks"].flip(-2)  # Volteo vertical en el eje de altura
                if "keypoints" in target:
                    keypoints = target["keypoints"]
